...
sftp = ssh.open_sftp()
sftp.get('/home/helm/expense-tracker/expense_dashboard.html',
...
moonspoon_pattern = r'(<div id="moonspoon-tab" class="tab-content">.*?<div class="container">.*?<div class="divider"></div>)'
        ...
        if len(parts) == 2:
            ...
            if len(moonspoon_content) == 2:
                ...
                if '<div class="divider"></div>' in moonspoon_body:
                    ...
                    html = parts[0] + '<div id="moonspoon-tab" class="tab-content">' + moonspoon_body + '</div>\n    <!-- Personal Tab' + moonspoon_content[1]
...
sftp = ssh.open_sftp()
sftp.put(r'C:\Users\ASUS\main_dashboard_with_tools.html',
...
sftp = ssh.open_sftp()
sftp.put(r'C:\Users\ASUS\sessions_dashboard_updated.html',
...
sftp = ssh.open_sftp()
sftp.put(r'C:\Users\ASUS\pricing_calculator_updated.html',
...
stdin, stdout, stderr = ssh.exec_command('cd /home/helm/expense-tracker && nohup node dashboard_api.js > dashboard.log 2>&1 &')
stdout.channel.recv_exit_status()
time.sleep(3)
