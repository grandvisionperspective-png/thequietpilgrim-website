...
stdin, stdout, stderr = ssh.exec_command('cp /var/www/expense-system/backend/main.py /var/www/expense-system/backend/main.py.backup-before-sessions')
stdout.channel.recv_exit_status()
print('  ✅ Backup created')

# Upload new version
sftp.put(r'C:\Users\ASUS\main_py_with_sessions_clean.py', '/var/www/expense-system/backend/main.py')
sftp.close()
print('  ✅ Uploaded')
...
tests = [
    ('GET /api/sessions (all)', 'curl -s "http://localhost:8000/api/sessions?status=all"'),
    ('GET /api/sessions (personal only)', 'curl -s "http://localhost:8000/api/sessions?entity=personal&status=all"'),
