...
async def health_check():
    ...
    return {
        ...
        "services": {
            ...
            "telegram_notifications": "enabled" if getattr(settings, "ENABLE_TELEGRAM_NOTIFICATIONS", False) else "disabled"
        },
        "entities": {
            "personal": {"sheet": settings.PERSONAL_SHEET_ID, "folder": settings.PERSONAL_DRIVE_FOLDER_ID},
            "business": {"sheet": settings.BUSINESS_SHEET_ID, "folder": settings.BUSINESS_DRIVE_FOLDER_ID},
            "moonspoon": {"sheet": settings.MOONSPOON_SHEET_ID, "folder": settings.MOONSPOON_DRIVE_FOLDER_ID}
        },
        "timestamp": datetime.now().isoformat()
    }
...
async def process_receipt_background(
    ...
    try:
        ...
        extracted_data = await extraction_service.extract_receipt_data(
...
async def get_receipts_needing_review(entity: Optional[str] = None):
    ...
    try:
        ...
        for ent in entities_to_check:
            ...
            receipts = await sheets_service.get_rows(
                spreadsheet_id=sheet_id,
                range_name='Receipts!A:Q'
            )
...
async def get_moonspoon_sessions():
    ...
    try:
        ...
        sessions = await sheets_service.get_rows(
            spreadsheet_id=sheet_id,
            range_name='Sessions!A:I'
        )
...
async def get_pricing_options():
    ...
    return {
        "service_types": [
            {"value": "drop_off_buffet", "label": "Drop-off Buffet", "description": "Food prepared and dropped off"},
            {"value": "set_up", "label": "Set Up", "description": "Food delivered and set up on site"},
            {"value": "plated_service", "label": "Plated Service", "description": "Individual plated meals served"},
            {"value": "full_service", "label": "Full Service", "description": "Complete event service with chef on-site"}
        ],
        "event_types": [
            {"value": "weekly_menu", "label": "Weekly Menu", "description": "Regular client weekly meals"},
            {"value": "family_meal", "label": "Family Meal", "description": "Private family dining"},
            {"value": "villa_dinner", "label": "Villa Dinner", "description": "Villa private event"},
            {"value": "retreat_lunch", "label": "Retreat Lunch", "description": "Retreat/group lunch"},
            {"value": "retreat_dinner", "label": "Retreat Dinner", "description": "Retreat/group dinner"},
            {"value": "boutique_event", "label": "Boutique Event", "description": "Special boutique event"}
        ],
        "staff_models": [
            ...
            {"value": "chef_plus_service_team", "label": "Chef + Full Service Team", "cost": 2500000}
        ],
        "sourcing_options": [
        ...
        for entity, sheet_id in entities.items():
            try:
                ...
                dashboard["budgets"][entity] = {
                    ...
                    "status": "exceeded" if percentage >= 100 else "critical" if percentage >= 85 else "warning" if percentage >= 75 else "healthy"
                }
...
async def get_expense_stats(entity: str):
    ...
    try:
        ...
        top_merchants = [
            {"name": m, "total": v}
            for m, v in sorted(merchants.items(), key=lambda x: x[1], reverse=True)[:10]
        ]

        # Category breakdown
        category_breakdown = [
            {"name": c, "total": v}
            for c, v in sorted(categories.items(), key=lambda x: x[1], reverse=True)
        ]
